...
const SideBar: React.FC = () => {
  ...
  return (
    <div
      className={`bg-gray4 text-white h-screen ${
        open ? "w-72" : "w-20"
      } duration-300 relative border-r-[5px] border-white`}
    >
      <BsArrowLeftShort
        className={`bg-white text-gray4 text-3xl rounded-full z-20 absolute -right-3 top-5 border border-gray4 cursor-pointer ${
          !open && "rotate-180"
        }`}
      ...
      <div className="inline-flex items-center p-4">
        <IoIosFitness
          className={`w-[36px] h-auto cursor-pointer mr-2 duration-500 ${
            !open && "rotate-[360deg]"
          }`}
        />
        <h1
          className={`text-2xl origin-left text-white font-sans font-bold italic duration-300 ${
            !open && "scale-0"
          }`}
        >
          FitFlowz
          ...
          <li key={index}>
            <Link
              ...
              className={`text-white font-sans font-medium text-xl flex items-center gap-x-4 cursor-pointer p-4 mt-2 ml-1 hover:bg-green5 hover:text-green3 hover:bg-opacity-10 hover:rounded-full w-max ${
                isActive(menu.link)
                  ? " bg-green5 bg-opacity-10 rounded-full w-max text-green2"
                  : ""
              }`}
            >
