...
import { CountClasses, CountMembers,CountEquipments, CountStaffs } from "../../../service/https";
...
const Stat: React.FC = () => {
    const [counts, setCounts] = useState<{ classCount: number | null; memberCount: number | null; equipmentCount: number | null; staffCount: number | null }>({
        classCount: null,
        memberCount: null,
        staffCount: null,
        equipmentCount: null,
    ...
    useEffect(() => {
        const fetchCounts = async () => {
            const [classData, memberData, staffData,equipmentData] = await Promise.all([CountClasses(), CountMembers(), CountStaffs(), CountEquipments()]);
            setCounts({
                classCount: classData?.count ?? null,
                memberCount: memberData?.count ?? null,
                staffCount: staffData?.count ?? null,
                equipmentCount: equipmentData?.count ?? null,
            });
        };
    ...
    return (
        <div className="col-span-3 md:col-span-2 flex flex-col items-center md:items-start gap-4 pt-10 px-2">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pb-8 mx-auto">
                ...
                <StatCard count={counts.equipmentCount ? counts.equipmentCount.toString() : "Loading..."} Icon={IoIosFitness} label="Equipments" rotateIcon="-rotate-45" />
                <StatCard
